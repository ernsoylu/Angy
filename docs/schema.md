...
| `block_index` | Projection of the **actionable** nodes inside pages (ADR 0001, V2 H1) — page links and mentions today, tasks/macros next. Worker-written alongside rendered_html, never by the editor; not a MATERIALIZED VIEW, and **not** the block table hard rule 2 forbids — it is derived, disposable and rebuildable from the Y.Doc. One row per actionable node, never one per block | `page_id`, `ord` (document order); PK (page_id, ord); `kind` (block_ref_kind: PAGE_LINK, MENTION), `target_page_id`, `target_user_id`, `payload` jsonb (links and mentions carry `{label}` — the text *as rendered*, which is what the stale-label refresh compares against); indexes on target_page_id (backlinks) and target_user_id (mention inbox) |
