...
- **E2E (playwright)**: reader SSR render, edit-on-click mount, multi-user collab session, page history restore. Run against the pnpm docker:up stack with all four apps booted, and serve **web from a production build, never `next dev`** — under dev mode StrictMode's double-invoked effects kill the collab socket before it establishes, so all 11 editor specs fail on "1 live connection" while the reader specs pass, looking exactly like a broken editor. **The reader is streamed, so a locator on rendered content can transiently match twice** — the real node plus React's hidden `id="S:…"` template — and strict mode then fails the spec that happened to sample mid-swap. Go through `articleBody()`/`pageTitle()` in `e2e/helpers.ts`; never write a bare `.article-prose` or `h1`. Two more races have the same shape: an assertion that follows a **checkpoint** must poll with `page.reload()` (the projection rebuild is a worker job), and one that follows an **optimistic control** (star, reorder) must `waitForResponse` before reloading, or the server render beats the write. A lone failure that passes on re-run of the same commit is a flake, not a regression.
...
- **BullMQ custom job ids must not contain ":"** — it's reserved for key namespacing. And a custom job id needs `removeOnComplete`/`removeOnFail` with it: BullMQ retains completed jobs by default, and adding an id that still exists is a **silent** no-op, not an error. A recurring sweep keyed by entity (the projection reconciler) therefore repairs each page exactly once, then reports it stale forever and alerts that the worker is wedged while the worker is healthy.
...
- frontend.pen — Pencil design file, the source of truth for **every** screen, with no exceptions: reader, editor, search, history & diff, space home, sign-in, share & permissions, attachments, trash, move page, system states, space settings, space-settings modal, comments, database view & properties, tasks board, mentions, import, inbox and backlinks rail — plus design-system frames (foundations, components, responsive, interaction & density), each in light and dark. A new screen gets its frame in the same wave that builds it; a screen without one is drift, not a convention. Build apps/web to match these frames. **Edit it only through the Pencil MCP** (`mcp__pencil__*`) — never Read/Grep/Write the file. In that editor, `Copy` an existing node and customise it; freshly `Insert`ed subtrees have been observed not to render (`Replace` them afterwards, which does), a `Copy`'s `descendants` overrides can silently not apply (re-`Update` the copied children by id), `strikethrough` on text is dropped entirely, and `ctx.bounds` can lag a mutation — so verify with `get_screenshot`, not with bounds.
